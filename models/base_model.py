    ...
    def __init__(self, config):
        ...
        if use_new_suffix:
            for output_features in block_features:
                ...
                fc = modules.FullyConnected(2*output_features, self.config.num_classes, config.architecture.dropout, activation_fn=None)
                self.fc_layers.append(fc)

        else:  # use old suffix
            # Sequential fc layers
            self.fc_layers.append(modules.FullyConnected(2*block_features[-1], 512, config.architecture.dropout))
            self.fc_layers.append(modules.FullyConnected(512, 256, config.architecture.dropout))
            self.fc_layers.append(modules.FullyConnected(256, self.config.num_classes, config.architecture.dropout,activation_fn=None))
