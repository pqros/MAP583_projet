...
class RegularBlock(nn.Module):
    ...
    def __init__(self, config, in_features, out_features):
        ...
        self.mlp1 = MlpBlock(in_features, out_features, config.architecture.depth_of_mlp, config.architecture.dropout)
        self.mlp2 = MlpBlock(in_features, out_features, config.architecture.depth_of_mlp, config.architecture.dropout)

        self.skip = SkipConnection(in_features + out_features, out_features, config.architecture.dropout)
...
class MlpBlock(nn.Module):
    ...
    def __init__(self, in_features, out_features, depth_of_mlp, dropout, activation_fn=nn.functional.relu):
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        for i, conv_layer in enumerate(self.convs):
            out = F.dropout(self.activation(conv_layer(out, )), p=self.dropout)
...
class SkipConnection(nn.Module):
    ...
    def forward(self, in1, in2):
        ...
        out = F.dropout(self.conv(out), p=self.dropout)
        return out


class FullyConnected(nn.Module):
    def __init__(self, in_features, out_features, dropout, activation_fn=nn.functional.relu):
        super().__init__()
        self.dropout = dropout
    ...
    def forward(self, input):
        ...
        if self.activation is not None:
            out = F.dropout(self.activation(out), p=self.dropout)
